...
// nodeLocalLoadBalancing, an envoy on every worker's own loopback
...
func (b Builder) config(ctx context.Context, d cluster.Deps, n lab.Node) error {
	...
	// "custom" hands the pod network to the network installer; the
	// distribution's own is kuberouter, which a default row keeps.
	provider := "custom"
...
spec:
  ...
  network:
    ...
    podCIDR: %[4]s
    serviceCIDR: %[5]s
    # Every worker gets an envoy on its own loopback across all the
    # control planes, which is k0s's answer to the question this
    # product's kubeadm site needs a static pod to answer. Nothing
    # here adds a second balancer on top of it.
...
`, addr, strings.Join(sans, ", "), provider, d.PodCIDR, d.SvcCIDR)
...
// /run/k0s/nllb and aims at the envoy on the node's own loopback,
...
func (b Builder) KubeletInvariant(ctx context.Context, d cluster.Deps) error {
	...
	for _, n := range cluster.SiteNodes(d.Topology) {
		...
		default:
			return fmt.Errorf("%s's kubelet dials %s, not its own nllb envoy: "+
