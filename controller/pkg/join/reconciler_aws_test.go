...
func TestReconcile_InfraProviderValidateErrorBlocksBootstrapSecretCreation(t *testing.T) {
	...
	machine.SetName("hilton-cloud-worker-jarvistam-0")
	machine.SetNamespace("default")
	machine.SetLabels(map[string]string{"cluster.x-k8s.io/cluster-name": "hilton-jarvistam"})
	_ = unstructured.SetNestedField(machine.Object, "hilton-cloud-worker-jarvistam-0", "spec", "infrastructureRef", "name")
	...
	if err := c.Get(context.Background(), client.ObjectKey{Namespace: "default", Name: "hilton-cloud-worker-jarvistam-0-bootstrap"}, &corev1.Secret{}); !apierrors.IsNotFound(err) {
