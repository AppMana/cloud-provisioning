	...
	for _, p := range peers {
		if p.PublicKey == the on-prem hostPub {
			...
			if !equalStrings(p.WGAllowedIPs, want) {
				t.Errorf("the on-prem host WGAllowedIPs = %v, want both tunnel address AND cluster VIP: %v", p.WGAllowedIPs, want)
			}
			if p.RouteHost != "10.100.0.1" {
				t.Errorf("the on-prem host RouteHost = %q, want just the tunnel address, no cluster VIPs", p.RouteHost)
			}
		}
	}
	if !found {
...
func TestOnPremPeers_DualStackClusterVIPsGetCorrectPrefixLengths(t *testing.T) {
	...
	for _, p := range peers {
		...
		if !equalStrings(p.WGAllowedIPs, want) {
			t.Errorf("the on-prem host WGAllowedIPs = %v, want %v", p.WGAllowedIPs, want)
		}
		return
	}
