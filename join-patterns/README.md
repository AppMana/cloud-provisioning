...
| RKE2 | same agent balancer as k3s, registering through the supervisor on 9345 | off, same render test. Site caveat, RKE2's own: a sustained partition of a server node wedges its etcd beyond the distribution's unaided recovery (fatal on lost leader election orphans the etcd static pod, whose log consumer dies with containerd; the survivor heartbeats raft but cannot serve, and every restart dies reading it). Measured in the lab, manual recovery verified, known upstream (rancher/rke2#4510, #4479, #7155); k3s embeds etcd in-process and passes the same outage |
