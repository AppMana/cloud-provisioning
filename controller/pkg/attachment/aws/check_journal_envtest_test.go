...
func TestAPIJournalRetainsSourceSettingUntilLastLeaseIsReleased(t *testing.T) {
	...
	if err := c.Create(ctx, &corev1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: "source-checks"}}); err != nil {
		t.Fatal(err)
	}
	r, e, _, target := checkFixture(t)
	e.sourceCheck = true
	journal := ConfigMapCheckJournal{Client: c, Namespace: "source-checks"}
	r.Journal = journal
	...
	if done, err := r.Release(ctx, "2022", target); err != nil || !done || e.sourceCheck {
	...
	if done, err := r.Release(ctx, "2025", target); err != nil || !done || !e.sourceCheck {
	...
	if err := c.Get(ctx, journal.key(r.key(target)), &corev1.ConfigMap{}); !apierrors.IsNotFound(err) {
		t.Fatalf("released source-check retained finalizer: %v", err)
	}
}
