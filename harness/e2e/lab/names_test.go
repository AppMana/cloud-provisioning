...
func TestEveryInterfaceNameFitsTheKernelsLimit(t *testing.T) {
	...
	for _, n := range topo.Nodes {
		...
		if got := ManagementBridge(n.Name); len(got) > MaxInterfaceName {
			t.Errorf("management bridge %q is %d characters, and the kernel takes %d",
				got, len(got), MaxInterfaceName)
		}
	}
