...
# the node's actual Calico state.
...
echo "  running at $EC2_IP -- cloud-init installs wireguard-tools and blocks in wg-pullup's WAIT_FOR loop until the on-prem host dials in (expect ~30-90s for apt alone)"
...
ssh -o BatchMode=yes "$ONPREM_SSH_HOST" bash -s -- "$ONPREM_KEY" "$EC2_PUB" "$EC2_IP" "$WG_NET" "$TEST_NET" < ./remote-dialer-setup.sh

echo "$ONPREM_KEY" > "$STATE_DIR/onprem.key"; chmod 600 "$STATE_DIR/onprem.key"
...
if ssh -o BatchMode=yes "$ONPREM_SSH_HOST" "ping -c2 -W3 -I ${TEST_NET}.1.1 ${TEST_NET}.2.1 >/dev/null 2>&1"; then
  echo "  OK: ${TEST_NET}.1.1 (the on-prem host) -> ${TEST_NET}.2.1 (ec2) over the real WAN"
else
  echo "  FAILED"
  exit 1
fi
