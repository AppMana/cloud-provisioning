...
func TestAPIJournalRetainsIngressUntilLastLeaseIsReleased(t *testing.T) {
	...
	if err := c.Create(ctx, &corev1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: "ingress-checks"}}); err != nil {
		t.Fatal(err)
	}
	r, e, _, target := ingressFixture(t)
	journal := ConfigMapIngressJournal{Client: c, Namespace: "ingress-checks"}
	r.Journal = journal
	e.journal = journal
	...
	if done, err := r.Release(ctx, "2022", target); err != nil || !done || len(e.rules) != 1 {
	...
	if done, err := r.Release(ctx, "2025", target); err != nil || !done || len(e.rules) != 0 {
	...
	if err := c.Get(ctx, journal.key(r.key(target)), &corev1.ConfigMap{}); !apierrors.IsNotFound(err) {
		t.Fatalf("released ingress retained finalizer: %v", err)
	}
}
