...
func (p *Product) Install(ctx context.Context, opts Options, dialerSHA string) error {
	...
	args := []string{
		...
		"--set", "image.repository=" + strings.Split(ControllerImage, ":")[0],
		"--set", "image.tag=" + strings.Split(ControllerImage, ":")[1],
		"--set", "image.pullPolicy=Never",
		"--set", "dialerImage.repository=" + strings.Split(DialerImage, ":")[0],
...
func (p *Product) carryChart(ctx context.Context) error {
	chart := filepath.Join(p.RepoDir, "charts", "cloud-provisioning")
	if _, err := os.Stat(chart); err != nil {
		return fmt.Errorf("no chart at %s: %w", chart, err)
	}
